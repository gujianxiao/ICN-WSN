...
//	uses interface Receive as DataRec;
	...
	uses interface Timer<TMilli> as CmRefresh;
...
//					memcpy(&(send->msgName), &(pitTable[i].pitName), sizeof(name));
					if((call AMSend.send(pitTable[i].comeId, msg, sizeof(Msg))) == SUCCESS) {
						#ifndef DEBUG
						printf("find the matched item in the pit table,transmit the packet to :%d\n", pitTable[i].comeId);
//						printf("the data hop is %d\n",receive->hop);
						printfflush();
						#endif
...
//					pitNum--;
	...
	event void AMControl.startDone(error_t error) {
		...
		if(error != SUCCESS) {
			...
			call  CmRefresh.startPeriodic(CMTIME);
...
//		printfflush();
	...
	//modify by cb
	event void CmRefresh.fired() {
		// TODO Auto-generated method stub
		for(i = 0; i < CMMAX; i++) 
			memset(&cacheMap[i], 0, sizeof(cmIt));
		//memset(pitTable, 0, sizeof(pitIt) * PITMAX);
		//csNum = 3;
		//mapFlag=0;
		cacheMapSize=0;
...
//		printfflush();
		#endif
		dbg("CmRefresh","CmRefresh　: refresh the cacheMap!\n");
	}
		...
		if(len == sizeof(BroMsg)) {
			if(broRec->msgType == BRO) {
				location_merge(thisNode.nodeAbility, broRec->nodeAbility, &(thisNode
						.nodeAbility));
				sons[addr]=broRec->sonsnum;
				sons_update();
				Betw=(float)sonsnum/15;
...
//		dbg("MsgRec","sizeof(Msg) = %d\n",sizeof(Msg));
		if(len == sizeof(Msg)) {
			if(receive->msgType == IN) {
				...
				printf("The IN's range:/(%d,%d)/(%d,%d) hop: %d\n", a.leftUp.x, a.leftUp.y, a
				.rightDown.x, a.rightDown.y,receive->hop);
...
//				printfflush();
			...
			if(receive->msgType == DATA) {
				#ifndef DEBUG
				location a=(receive->msgName).ability;
				printf("receive a data Packet from %d hop is %d\n",addr,receive->hop);
...
//				printf("data:%d\n",receive->data);
	...
	bool cs_query_In(Msg * Rec, message_t * rec) {
		...
		for(i = 0; i < csNum; i++) {
			...
			//		printf("In cs_query_In hop is %d\n",send->hop);
					printfflush();
                    
					if(call AMSend.send(addr, rec, sizeof(Msg)) == SUCCESS) {
//						
...
//			printfflush();
	...
	bool cs_query_Data(Msg * ndwRec, message_t * rec) {
        ...
        float Ratio=0;
        uint8_t maxTag=0;
        uint8_t allReplacedCount=0;
        uint8_t popularity=0;
        float caRePro=0;
        bool existFlag=FALSE;
		for(i = 0; i < csNum; i++) {//if in cs then return
...
//					printfflush();
		...
		for(i=0; i<cacheMapSize; i++)//cache decision
		{
		if(ability_equal((cacheMap[i].msgName).ability, (ndwRec->msgName).ability)){
		   if(cacheMap[i].replacedCount==0) {
			cacheMap[i].replacedCount=0;                   
			break;//never be replaced then cache it
		   }    
	           Ratio=(float)cacheMap[i].replacedCount/(float)allReplacedCount;
	           dePro=(float)Betw/(float)Ratio;//need to be normalized now it's wrong
	           proInt=dePro*100;
	           randInt=rand()%100;
	           if(randInt<proInt) {
                     randInt=0;
                     break;//cache it with proability pro		                
		   }
	           else return TRUE;	//not cahe in 1-pro    
		}		
		}
...
//			csTable[csNum].hop = ndwRec->hop;
			csNum++;
		}
		if(csNum >= csmax) {//cache replacement algorithm:popularity and content value
			
			for(i=0;i<csmax;i++){//find the max caRePro to replace
				
				if(allTouchedCount==0){ 
				popularity=0;
				}					
				else {
					popularity=csTable[i].touched/allTouchedCount;
				}
				
				Index=abs(Betw-popularity);
				tempPro=1-pow(E,Index);
				caRePro=(float)(tempPro*100);
				//csTable[i].caRePro=caRePro;
				if(caRePro>maxRePro) 
					{
					  maxRePro=caRePro;
				          maxTag=i;
			...
			//add the repalce count
			for(i=0;i<cacheMapSize;i++){//find item whether in the cacheMap
				if(ability_equal((csTable[maxTag].csName).ability, (cacheMap[i].msgName).ability)){ 
					cacheMap[i].replacedCount++;
				        allReplacedCount++;
					existFlag=TRUE;
					break;
					}
			...
			if(!existFlag){//not in the cacheMap
					memset(&cacheMap[cacheMapSize], 0, sizeof(cmIt));
					memcpy(&(cacheMap[cacheMapSize].msgName), &(ndwRec->msgName), sizeof(name));
					cacheMap[cacheMapSize].replacedCount=1;
					cacheMapSize++;
					//mapFlag=(mapFlag%CMMAX)+1;
				}
				

				
			//cache it in cacheTble;
			memset(&csTable[maxTag], 0, sizeof(csIt));
			memcpy(&(csTable[maxTag].csName), &(ndwRec->msgName), sizeof(name));
			csTable[maxTag].data = ndwRec->data;
	//		csTable[maxTag].hop = ndwRec->hop;
			csTable[maxTag].weight = 3;
