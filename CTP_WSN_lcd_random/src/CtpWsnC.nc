...
//				dbg("fib_send","fib_send : fibcount= %d\n",fibCount);
				...
				send->msgType = IN;
				//add the hop to count
				//send->hop++;
				send->hhop=receive->hhop+1;
...
//						dbg("DataSend","DataSend:in cs_query_IN ,find the matched item in cs table!Send the Data msg to %d\n",addr);
						#ifndef DEBUIG
					//printf("Hop for this interest: %d\n",Rec->hop);
...
//					printfflush();
		...
		if(ndwRec->hop==1){//cache decision strategy: lcp
		ndwRec->hop=0;		
		//rec->hop=0;
		}
		else return TRUE;
		...
		if(csNum >= CSMAX) {//cache replacement algorithm：RANDOM
			randInt=rand()%15;
			memset(&(csTable[randInt]), 0, sizeof(csIt));
			memcpy(&(csTable[randInt].csName), &(ndwRec->msgName), sizeof(name));
			csTable[randInt].data = ndwRec->data;
			csTable[randInt].weight = 3;
